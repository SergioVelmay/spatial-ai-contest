...
def GetPalmRectFromRegion(frame: np.ndarray, region: HandRegion):
...
def CalculatePalmRectFromRegion(color_image: np.ndarray, region: HandRegion):
    x1, y1, x2, y2 = GetPalmRectFromRegion(color_image, region)
    y_percentage = int((y2-y1)*0.125)
    x_percentage = int((x2-x1)*0.125)
    xMin = x1-x_percentage
    yMin = y1-(2*y_percentage)
    xMax = x2+x_percentage
    yMax = y2
    return xMin, yMin, xMax, yMax
